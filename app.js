...
async function startQueueProcessor() {
    ...
    while (jobQueue.length > 0) {
        ...
        const { jobId, action, email, magicLink } = job;
        ...
        while (attempts < 2 && !isSuccess) {
            attempts++;
            try {
                console.log(`\n${C.brightCyan}[QUEUE] Mengeksekusi ${action} untuk ${email} (Attempt ${attempts}/2)${C.reset}`);
        ...
        if (action === 'verify_and_claim' || action === 'claim_only') {
            ...
            const failMessage = applyData.message || applyData.error || executionResult?.message || executionResult?.error || "Gagal menerapkan premium";
            ...
            jobStore.set(jobId, {
                ...
                details: { verification: executionResult?.verif_res || null, apply_response: applyData, steps_completed: executionResult?.step },
    ...
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            if ((pathname === '/api/send' || pathname === '/api/verify' || pathname === '/api/claim') && method === 'POST') {
                ...
                startQueueProcessor(); // Trigger background queue processor
                return;
            }
